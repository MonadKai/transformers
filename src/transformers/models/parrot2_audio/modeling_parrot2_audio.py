...
from .configuration_parrot2_audio import Parrot2AudioConfig, Parrot2AudioEncoderConfig
...
# PARROT2AUDIO_START_DOCSTRING = r"""
...
# @add_start_docstrings(
#     "The bare Parrot2Audio Model outputting raw hidden-states without any specific head on top.",
#     PARROT2AUDIO_START_DOCSTRING,
# )
# class Parrot2AudioPreTrainedModel(PreTrainedModel):
#     config_class = Parrot2AudioConfig
...
# PARROT2AUDIOENCODER_START_DOCSTRING = r"""
#     This model inherits from [`PreTrainedModel`]. Check the superclass documentation for the generic methods the
#     library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
#     etc.)
...
#     Parameters:
#         config ([`Parrot2AudioEncoderConfig`]):
#             Model configuration class with all the parameters of the model. Initializing with a config file does not
#             load the weights associated with the model, only the configuration. Check out the
#             [`~PreTrainedModel.from_pretrained`] method to load the model weights.
# """
...
# @add_start_docstrings(
#     """The audio model from Parrot2Audio without any head or projection on top.""",
#     PARROT2AUDIOENCODER_START_DOCSTRING,
# )
# class Parrot2AudioEncoder(Parrot2AudioPreTrainedModel):
#     """
#     Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
#     [`Parrot2AudioEncoderLayer`].
...
#     # Ignore copy
#     config_class = Parrot2AudioEncoderConfig
#     main_input_name = "input_features"
#     _no_split_modules = ["EncoderLayerSANM"]

#     def __init__(self, config: Parrot2AudioEncoderConfig):
...
#         """
#         Computes the output length of the parrot2 audio encoder
#         """
#         return input_lengths, input_lengths


Parrot2AudioPreTrainedModel = ParrotSenseVoicePreTrainedModel
Parrot2AudioEncoder = ParrotSenseVoiceEncoder
...
class ParrotQwen3ForCausalLM(Qwen3ForCausalLM):
    _tied_weights_keys = None


PARROT2AUDIO_INPUTS_DOCSTRING = r"""
    Args:
...
class Parrot2AudioForConditionalGeneration(Parrot2AudioPreTrainedModel, GenerationMixin):
    def __init__(self, config: Parrot2AudioConfig):
        super().__init__(config)
        self.audio_tower = Parrot2AudioEncoder._from_config(config.audio_config)
        self.multi_modal_projector = Parrot2AudioMultiModalProjector(config)
        self.vocab_size = config.text_config.vocab_size
        self.language_model = ParrotQwen3ForCausalLM._from_config(config.text_config)
    ...
    @add_start_docstrings_to_model_forward(PARROT2AUDIO_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Parrot2AudioCausalLMOutputWithPast, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
