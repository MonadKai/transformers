...
from transformers.utils import TransformersKwargs, add_start_docstrings, auto_docstring, can_return_tuple, logging
from transformers.models.qwen3.modeling_qwen3 import Qwen3ForCausalLM

from ...modeling_utils import ALL_ATTENTION_FUNCTIONS, PreTrainedModel
from .configuration_funasr_nano import FunasrNanoSenseVoiceConfig, FunasrNanoConfig
from .funasr_modules import make_pad_mask
from .funasr_modules import EncoderLayer, MultiHeadedAttention, PositionwiseFeedForward
from .sensevoice_encoder import SenseVoiceEncoderSmall


logger = logging.get_logger(__name__)

_CONFIG_FOR_DOC = "FunasrNanoConfig"
...
SENSEVOICE_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`FunasrNanoSenseVoiceConfig`]):
...
@add_start_docstrings(
    "The bare FunasrNanoSenseVoice Model outputting raw hidden-states without any specific head on top.",
    SENSEVOICE_START_DOCSTRING,
)
class FunasrNanoSenseVoicePreTrainedModel(PreTrainedModel):
    config_class = FunasrNanoSenseVoiceConfig
...
SENSEVOICEENCODER_START_DOCSTRING = r"""
    This model inherits from [`PreTrainedModel`]. Check the superclass documentation for the generic methods the
    library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
    etc.)
...
class FunasrNanoSenseVoiceEncoder(FunasrNanoSenseVoicePreTrainedModel):
    ...
    def __init__(self, config: FunasrNanoSenseVoiceConfig):
...
class FunasrNanoMultiModalProjector(nn.Module):
    def __init__(self, config: FunasrNanoConfig):
        ...
        self.linear1 = nn.Linear(self.encoder_dim * self.k, self.ffn_dim)
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(self.ffn_dim, self.llm_dim)
        self.n_layer = config.n_layer
        assert self.n_layer > 0, "n_layer must be greater than 0"
        ...
        self.blocks = nn.ModuleList(
            [
                EncoderLayer(
                    self.llm_dim,
                    MultiHeadedAttention(self.attention_heads, self.llm_dim, self.attention_dropout_rate),
                    PositionwiseFeedForward(self.llm_dim, self.llm_dim // 4, self.dropout_rate),
                    self.dropout_rate,
                )
    ...
    def forward(self, x: torch.Tensor, ilens: Optional[torch.Tensor] = None) -> torch.Tensor:
...
class FunasrNanoPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        # important: this ported version of FunasrNano isn't meant for training from scratch - only
        # inference and fine-tuning - so the proper init weights code has been removed
...
@auto_docstring(
    custom_intro="""
    The FunasrNano model, which consists of a audio backbone and a language model.
    """
)
class FunasrNanoForConditionalGeneration(FunasrNanoPreTrainedModel, GenerationMixin):
    def __init__(self, config: FunasrNanoConfig):
        super().__init__(config)
        self.audio_tower = FunasrNanoSenseVoiceEncoder._from_config(config.audio_config)
    ...
    def forward(
    ...
    ) -> Union[tuple, FunasrNanoCausalLMOutputWithPast]:
        r"""
        ...
        >>> model = FunasrNanoForConditionalGeneration.from_pretrained("/path/to/funasr-nano-0.5b")
        >>> processor = AutoProcessor.from_pretrained("/path/to/funasr-nano-0.5b")

        >>> prompt = "<|startofspeech|>!<|endofspeech|>Generate the caption in English:"
        ...
        if inputs_embeds is None:
            ...
            if input_features is not None and input_ids.shape[1] != 1:
                ...
                if legacy_processing:
                    logger.warning_once(
                        "Expanding inputs for audio tokens in FunasrNano should be done in processing."
                    )
